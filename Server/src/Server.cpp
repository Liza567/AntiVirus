...
{
	std::u16string filePath = u"Monitors.ges";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;

	std::u16string header = reader.readU16String();
	if (header != u"Ganyushkina")
	{
		reader.close();
		return;
	}
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Ganyushkina");
...
{
	std::u16string filePath = u"Scanners.ges";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.ges"));
	threats = std::make_shared<ThreatList>(u"Threats.ges");
	threats->load();
